...
export default function RemoveStock() {
    const { stock, onRemoveStock } = useContext(ProductsContext);
    const [values, setValues] = useState(new Array(2)); 
    ...
    const onButtonClick = () => {
        if (values[0] != null && values[1] != null && values[2] != null){
            let checkFor = stock.find(item => item["code"] === values[0]);
    ...
    return (
        <div className="remove-stock-container container">
            ...
            <form className="remove-stock-form" onSubmit={e => e.preventDefault()}>
                ...
                <input type="number" name="items-bought" max={100000} placeholder="Number of items bought" required onChange={e => onInputChange(e.target.value, 2)} />
